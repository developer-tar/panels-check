  ...
  <div class="main-inner">
    ...
    <form method="POST" action="{{ route('admin.company.store') }}" enctype="multipart/form-data">
      ...
      <div class="col-span-2 md:col-span-1">
        ...
        <input type="text"
        class="w-full text-sm  bg-secondary/5 dark:bg-bg3 !border border-n30 dark:border-n500 rounded-3xl px-3 md:px-6 py-2 md:py-3"
        placeholder="Enter name" id="email" name="email"  value="{{ old(key: 'email') }}" />
        @error('email')
      <div class="text-red-500 text-sm mt-1">
      {{ $message }}
      </div>
    ...
    <option value="{{ config('constants.company.type.Coorportion') }}" {{ old('type') == config('constants.company.type.Coorportion') ? 'selected' : '' }}>Coorportion</option>
      ...
      <div class="col-span-2 md:col-span-1">
        ...
        <input type="text"
        class="w-full text-sm  bg-secondary/5 dark:bg-bg3 !border border-n30 dark:border-n500 rounded-3xl px-3 md:px-6 py-2 md:py-3"
        placeholder="Enter name" id="registration_number" name="registration_number" value="{{ old(key: 'registration_number') }}"/>
      ...
      <div class="col-span-2 md:col-span-1">
        ...
        <input type="text"
        class="w-full text-sm  bg-secondary/5 dark:bg-bg3 !border border-n30 dark:border-n500 rounded-3xl px-3 md:px-6 py-2 md:py-3"
        placeholder="Enter name" id="website_url" name="website_url" value="{{ old(key: 'website_url') }}" />
        @error('website_url')
      <div class="text-red-500 text-sm mt-1">
      {{ $message }}
      </div>
      ...
      <div class="col-span-2 ">
        ...
        placeholder="Enter Description..." rows="5" id="description" name="description" >{{ old(key: 'description') }}</textarea>
        @error('description')
      <div class="text-red-500 text-sm mt-1">
      {{ $message }}
      </div>
